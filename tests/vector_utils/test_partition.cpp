...
TEST_F(VectorPartition, T01_Lomuto_SortedList) {
    ...
    size_t pivot = VectorUtils::partition_lomuto(vec, 0, 4);
    EXPECT_EQ(pivot, 4);
...
TEST_F(VectorPartition, T05_Lomuto_TwoElementsSorted) {
    ...
    size_t pivot = VectorUtils::partition_lomuto(vec, 0, 1);
    EXPECT_EQ(pivot, 1);
...
TEST_F(VectorPartition, T07_Lomuto_InvalidLowHigh) {
    ...
    EXPECT_THROW(VectorUtils::partition_lomuto(vec, 2, 2), std::invalid_argument);
}

TEST_F(VectorPartition, T08_Lomuto_HighOutOfBounds) {
    std::vector<size_t> vec = {1, 2, 3};
    EXPECT_THROW(VectorUtils::partition_lomuto(vec, 0, 5), std::invalid_argument);
}
...
TEST_F(VectorPartition, T09_Hoare_SortedList) {
    std::vector<size_t> vec = {1, 2, 3, 4, 5};
    size_t pivot = VectorUtils::partition_hoare(vec, 0, 4);
    EXPECT_TRUE(pivot < 4);
    EXPECT_EQ(vec, (std::vector<size_t>{1, 2, 3, 4, 5}));
}
...
TEST_F(VectorPartition, T11_Hoare_Duplicates) {
    ...
    size_t pivot = VectorUtils::partition_hoare(vec, 0, 5);
    EXPECT_TRUE(pivot <= 5);
...
TEST_F(VectorPartition, T12_Hoare_InvalidLowHigh) {
    std::vector<size_t> vec = {1, 2, 3};
    EXPECT_THROW(VectorUtils::partition_hoare(vec, 3, 2), std::invalid_argument);
}

TEST_F(VectorPartition, T13_Hoare_HighOutOfBounds) {
    std::vector<size_t> vec = {1, 2, 3};
    EXPECT_THROW(VectorUtils::partition_hoare(vec, 0, 5), std::invalid_argument);
}
